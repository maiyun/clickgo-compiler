...
`clickgo --native --platform linux`에서는 `build.linux.icon` 또는 공통 `build.icon`에 PNG 하나를 지정하면 Linux 데스크톱용 여러 크기의 아이콘을 자동으로 생성합니다. 최소 256×256인 정사각형 PNG를 사용하세요. 512×512 또는 1024×1024를 권장합니다. 설정 예:
...
생성된 아이콘은 임시 파일이며 패키징 종료 후 정리됩니다. 원본 이미지와 프로젝트 설정은 변경하지 않습니다. 기존 아이콘 디렉터리도 다시 생성하지 않고 그대로 사용할 수 있습니다. 이 설정은 설치 후 앱 목록에 표시되는 아이콘용이며 Native 창의 실행 중 아이콘용이 아닙니다.
