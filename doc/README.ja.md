...
`clickgo --native --platform linux` では、`build.linux.icon` または共通の `build.icon` に単一の PNG を指定すると、Linux デスクトップ用の各サイズのアイコンを自動生成します。256×256 以上の正方形の PNG を使用してください。512×512 または 1024×1024 を推奨します。設定例：
...
生成したアイコンは一時ファイルで、パッケージ化の終了後に削除されます。元画像やプロジェクトの設定は変更しません。既存のアイコンディレクトリも再生成せずにそのまま使用できます。この設定はインストール後のアプリ一覧のアイコン用であり、Native ウィンドウの実行時アイコン用ではありません。
